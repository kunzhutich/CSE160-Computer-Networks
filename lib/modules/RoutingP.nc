...
//     // Declare the hashmap for storing link states
//     Hashmap<uint32_t, LinkInfo> linkStateMap;
...
implementation{
    ...
    command void Routing.lostNeighbor(uint16_t lost) {
        ...
        if(call linkStateMap.contains(linkKey)) {
            uint8_t cost = maxCost;  // Set cost to maximum (infinity)
