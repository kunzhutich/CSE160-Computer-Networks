...
implementation {
    ...
    uint8_t retries[WINDOW_SIZE] = {0};         // To avoid infinite retransmissions
    ...
    command error_t Transport.connect(socket_t fd, socket_addr_t *addr) {
        ...
        sockets[socketIndex].state = SYN_SENT;
        dbg(TRANSPORT_CHANNEL, "Socket %d transitioning to SYN_SENT\n", fd);

        sockets[socketIndex].dest = *addr;
        dbg(TRANSPORT_CHANNEL, "Socket %d initiating connection to %d:%d\n", fd, addr->addr, addr->port);

        // Send SYN packet
        makePack(&synPacket, TOS_NODE_ID, addr->addr, 1, PROTOCOL_TCP | FLAG_SYN, 0, NULL, 0);
        if (call Sender.send(synPacket, addr->addr) != SUCCESS) {
            dbg(TRANSPORT_CHANNEL, "Failed to send SYN packet\n");
            return FAIL;
        }
    ...
    //             dbg(TRANSPORT_CHANNEL, "Socket %d sent FIN, transitioning to FIN_WAIT\n", fd);
    //             return SUCCESS;
    //         }
    //         return FAIL;
    //     }
    ...
    command error_t Transport.close(socket_t fd) {
        ...
        if (sockets[socketIndex].state == ESTABLISHED) {
            ...
            return FAIL;
        } else if (sockets[socketIndex].state == FIN_WAIT) {
            sockets[socketIndex].state = CLOSED;
            dbg(TRANSPORT_CHANNEL, "Socket %d closed after FIN_WAIT\n", fd);
            return SUCCESS;
        }
    ...
    //     uint8_t i;        
    //     for (i = base; i < nextSeqNum; i++) {
    //         if (!acked[i % WINDOW_SIZE] && retries[i % WINDOW_SIZE] < MAX_RETRIES) {
    ...
    //         } else if (retries[i % WINDOW_SIZE] >= MAX_RETRIES) {
    ...
    //     dbg(TRANSPORT_CHANNEL, "Processing ACK for Seq: %d\n", receivedPacket->seq);
        
    //     // if (receivedPacket->protocol == PROTOCOL_TCP) {
    //     if (receivedPacket->protocol == PROTOCOL_TCP && receivedPacket->seq < nextSeqNum) {
    ...
    //                 dbg(TRANSPORT_CHANNEL, "restarting timer for remaining unacknowledged packets");
    //             }
    //         }
    //     }
    ...
    event message_t* Receive.receive(message_t* msg, void* payload, uint8_t len) {
        ...
        dbg(TRANSPORT_CHANNEL, "Processing incoming packet: Seq %d\n", receivedPacket->seq);

        // Ensure it's a TCP packet and the sequence number is valid
        if (receivedPacket->protocol == PROTOCOL_TCP && receivedPacket->seq < nextSeqNum) {
            uint8_t ackSeq = receivedPacket->seq;

            if (receivedPacket->flags & FLAG_ACK) {
                dbg(TRANSPORT_CHANNEL, "ACK received for Seq: %d\n", ackSeq);
                ...
                if (ackSeq >= base && ackSeq < nextSeqNum) {
                    ...
                    if (base == nextSeqNum) {
                        ...
                        call RetransmitTimer.startOneShot(RETRANSMIT_TIMEOUT); // Restart timer
                    }
                }
            }

            if (receivedPacket->flags & FLAG_FIN) {
                uint8_t socketIndex = findSocket(receivedPacket->dest);

                dbg(TRANSPORT_CHANNEL, "FIN received for socket %d\n", receivedPacket->dest);
                
                if (socketIndex != MAX_NUM_OF_SOCKETS) {
                    sockets[socketIndex].state = TIME_WAIT;
                    dbg(TRANSPORT_CHANNEL, "Socket %d transitioning to TIME_WAIT\n", receivedPacket->dest);
                }
            }
        }
