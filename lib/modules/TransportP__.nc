...
implementation {
    ...
    command socket_t Transport.socket() {
        uint8_t i;
        dbg(TRANSPORT_CHANNEL, "I AM IN TRANSPORT.SOCKET RN\n");
        for (i = 0; i < MAX_NUM_OF_SOCKETS; i++) {
            if (sockets[i].state == CLOSED) {
                sockets[i].state = ALLOCATED;       // Initialize to CLOSED state
                sockets[i].flag = i + 1;            // Assign unique socket ID (starting from 1)
                sockets[i].src = 0;                 // Initialize source port
                sockets[i].dest.addr = 0;           // Initialize destination address
                sockets[i].dest.port = 0;           // Initialize destination port

                dbg(TRANSPORT_CHANNEL, "Socket %d allocated and initialized\n", i + 1);
                return i + 1;                       // Return socket ID (1-based index)
            }
        }
